...
import type { ChatModelId, OpenAIChatMessage } from "@chatbot/shared";
import { createLLMStream } from "../services/llm.js";

export const v1ChatRouter: IRouter = Router();
...
v1ChatRouter.post("/chat/completions", async (req, res) => {
  ...
  const model = (body.model as ChatModelId) || "GPT-OSS-120B";
  ...
  try {
    ...
    const errorPayload = {
      error: { message, type: "server_error" as const },
    };
